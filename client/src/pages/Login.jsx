...
import { Bug, Mail, Lock, Loader2, ArrowRight, ShieldCheck, Sparkles, Hexagon } from 'lucide-react';
...
const TopBee = ({ size = 40, animated = true }) => {
  ...
  return (
    ...
    >
      <defs>
        <filter id="megaKawaiiFuzzLogin" x="-50%" y="-50%" width="200%" height="200%">
          <feTurbulence type="fractalNoise" baseFrequency="0.4" numOctaves="3" result="noise" />
          <feDisplacementMap in="SourceGraphic" in2="noise" scale="2.5" />
        </filter>
        <linearGradient id="megaHoneyLogin" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <radialGradient id="kawaiiEyeLogin" cx="30%" cy="30%" r="70%">
          <stop offset="0%" stopColor="#4B5563" /><stop offset="60%" stopColor="#111827" /><stop offset="100%" stopColor="#000000" />
        </radialGradient>
        <radialGradient id="deepBlushLogin" cx="50%" cy="50%" r="50%">
      ...
      <motion.g animate={animated ? { scale: [1, 1.1, 1] } : {}} transition={{ duration: 2, repeat: Infinity }}>
        <circle cx="75" cy="75" r="30" fill="url(#megaHoneyLogin)" filter="url(#megaKawaiiFuzzLogin)" />
      ...
      <motion.g animate={animated ? { rotate: isWiggling ? [-8, 8, -8] : [-2, 2] } : {}} transition={{ duration: isWiggling ? 0.3 : 4, repeat: isWiggling ? 4 : Infinity }}>
        <circle cx="40" cy="60" r="35" fill="url(#megaHoneyLogin)" filter="url(#megaKawaiiFuzzLogin)" />
        <circle cx="40" cy="60" r="32" fill="url(#megaHoneyLogin)" fillOpacity="0.2" />
        <circle cx="15" cy="70" r="10" fill="url(#deepBlushLogin)" />
        <circle cx="65" cy="70" r="10" fill="url(#deepBlushLogin)" />
        <g>
          <circle cx="22" cy="55" r="16" fill="url(#kawaiiEyeLogin)" />
          ...
          <circle cx="58" cy="55" r="16" fill="url(#kawaiiEyeLogin)" />
...
export default function Login() {
  ...
  useEffect(() => {
    axios.get(`${API_BASE_URL}/settings.php`).then(res => {
      setRegistrationEnabled(parseInt(res.data.enable_registration ?? 1) === 1);
    });
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try { await login(email, password); navigate('/dashboard'); } catch (err) { setError(err.response?.data?.message || 'Invalid credentials'); } finally { setLoading(false); }
  };
        ...
        <div className="glass rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl shadow-amber-500/5 border-2 border-white flex flex-col md:flex-row bg-white/80 backdrop-blur-2xl">
          ...
          <div className="md:w-5/12 bg-amber-500 p-8 md:p-12 text-white flex flex-col justify-between relative overflow-hidden min-h-[250px]">
            <div className="relative z-10">
              ...
              <h1 className="text-3xl md:text-4xl font-black tracking-tighter leading-tight mb-4 uppercase">Secure <br/> Hive Portal</h1>
              <p className="text-amber-50 font-black uppercase tracking-widest text-[8px] md:text-[10px] opacity-80">Orchestrating digital excellence.</p>
            </div>
            <div className="relative z-10 mt-8 md:mt-16 space-y-4 md:space-y-6 hidden sm:block">
              {[ { icon: ShieldCheck, text: 'Neural Encryption' }, { icon: Sparkles, text: 'Sovereign Logic' } ].map((item, i) => (
          ...
          <div className="md:w-7/12 p-6 md:p-12 bg-white">
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="mb-8 flex justify-between items-start">
                <div>
                  <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight uppercase">Identity Link</h2>
                  <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest mt-1">Initialize your command session.</p>
                </div>
              ...
              {error && <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="p-4 bg-red-50 border border-red-100 text-red-600 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2"><div className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse" />{error}</motion.div>}
              
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email Terminal</label>
                <div className="relative group">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 group-focus-within:text-amber-500 transition-colors" />
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-xl focus:ring-4 ring-amber-50 focus:border-amber-500 focus:bg-white outline-none transition-all font-bold text-slate-700 placeholder:uppercase placeholder:text-[9px]" placeholder="enter email..." />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-xl focus:ring-4 ring-amber-50 focus:border-amber-500 focus:bg-white outline-none transition-all font-bold text-slate-700 placeholder:uppercase placeholder:text-[9px]" placeholder="••••••••" />
                </div>
              </div>

              <button type="submit" disabled={loading} className="w-full bg-slate-900 hover:bg-amber-500 text-white font-black py-4 md:py-5 rounded-[1.25rem] shadow-xl shadow-slate-100 transition-all flex items-center justify-center gap-3 disabled:opacity-70 group uppercase tracking-widest text-xs">
                {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Initialize Session <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></>}
              </button>
              ...
              {registrationEnabled && (
                <p className="text-center text-slate-400 text-[10px] font-black uppercase tracking-widest mt-6">New to hive? <Link to="/register" className="text-amber-600 font-black hover:underline ml-1">Create Colony</Link></p>
              )}
            </form>
          </div>
        </div>
        <p className="text-center text-slate-300 text-[9px] mt-8 font-black uppercase tracking-[0.4em]">© 2026 BEE CHAT PRO · HIVE SECURED</p>
      </motion.div>
    </div>
  );
}
