...
const API = 'http://localhost/Bee/server/api/conversations.php';
...
export default function ChatConsole() {
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] bg-white rounded-[2.5rem] border border-slate-200/50 overflow-hidden shadow-2xl shadow-slate-200/50">
      {/* Sidebar: Chat List */}
      <div className="w-96 border-r border-slate-100 flex flex-col bg-slate-50/30">
        <div className="p-8 border-b border-slate-100 bg-white">
          <h3 className="font-black text-2xl text-slate-900 tracking-tight mb-6">Conversations</h3>
          ...
          <div className="relative mb-6">
            ...
            <input 
              type="text" 
              placeholder="Search Visitor UID or ID..." 
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium outline-none focus:ring-2 ring-indigo-100 transition-all"
            />
          </div>
        ...
        {selectedChat ? (
          <>
            ...
            <header className="h-24 border-b border-slate-50 px-8 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                 <div className="px-4 py-2 bg-slate-50 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 border border-slate-100">
                    Status: <span className={selectedChat.chat_status === 'ended' ? 'text-red-500' : 'text-emerald-500'}>{selectedChat.chat_status}</span>
                 </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-6 bg-slate-50/20 custom-scrollbar">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-slate-300">
                  ...
                  <p className="text-sm font-bold">Waiting for conversation data...</p>
                </div>
              ) : (
                messages.map((msg, i) => {
                    ...
                    <div key={i} className={`flex ${isAgent ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] group`}>
                        ...
                        <div className={`flex items-center gap-2 mt-2 px-2 ${isAgent ? 'justify-end' : 'justify-start'}`}>
                          ...
                          {isAgent && <CheckCheck className="w-3 h-3 text-indigo-400" />}
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="p-8 bg-white border-t border-slate-50">
              <form onSubmit={handleSendMessage} className="flex items-center gap-4 bg-slate-50 p-3 pl-6 rounded-3xl border border-slate-200/50 focus-within:bg-white focus-within:ring-4 ring-indigo-50 transition-all">
                <input
                  type="text"
                  ...
                  className="flex-1 bg-transparent border-none text-sm font-semibold outline-none text-slate-900 disabled:opacity-50"
                  value={input}
                ...
                <button
                  ...
                  className="bg-indigo-600 text-white p-4 rounded-2xl shadow-xl shadow-indigo-100 hover:scale-105 active:scale-95 transition-all disabled:opacity-30 disabled:scale-100"
                >
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-20 text-center">
            <div className="w-32 h-32 bg-slate-50 rounded-[3rem] flex items-center justify-center mb-8 relative">
                ...
                <div className="absolute -top-2 -right-2 w-8 h-8 bg-indigo-100 rounded-2xl flex items-center justify-center">
                    <Search className="w-4 h-4 text-indigo-600" />
                </div>
            </div>
            <h3 className="text-2xl font-black text-slate-900 mb-2">Platform Chat Archives</h3>
      ...
      {selectedChat && (
        <aside className="w-96 border-l border-slate-50 bg-white flex flex-col overflow-y-auto no-scrollbar">
          <div className="p-10 text-center border-b border-slate-50">
            <div className="w-24 h-24 rounded-[2.5rem] bg-indigo-50 flex items-center justify-center text-indigo-600 font-black text-3xl mx-auto mb-6 shadow-xl shadow-indigo-50 border-4 border-white">
              {selectedChat.visitor_uid?.slice(-2)}
            </div>
                ...
                <div className="space-y-4">
                  {visitorHistory.map((h, i) => (
                    <button 
                      ...
                      className="w-full text-left p-4 rounded-2xl border border-slate-50 hover:border-indigo-100 hover:bg-indigo-50/30 transition-all group"
                    >
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[10px] font-black text-indigo-600">ID: {h.id}</span>
                        <span className="text-[10px] font-bold text-slate-400">{format(new Date(h.created_at), 'MMM d')}</span>
                      </div>
                      <p className="text-xs font-bold text-slate-700 truncate group-hover:text-indigo-700">{h.last_message || 'Closed ticket'}</p>
                    </button>
                  ))}
                </div>
              )}
