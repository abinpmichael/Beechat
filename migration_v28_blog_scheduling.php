...
try {
    ...
    // 3. Define the 5 scheduled SEO blog posts
    $scheduledPosts = [
        [
    ...
    foreach ($scheduledPosts as $post) {
        ...
        if ($exists == 0) {
            ...
            echo "Seeded post: '{$post['title']}' (scheduled for {$post['published_at']}).\n";
        } else {
            echo "Post with slug '{$post['slug']}' already exists. Skipping.\n";
        }
    }
